...
router.post("/admin/change-password", requireAdmin, async (req, res) => {
  try {
    ...
    // 2. Changed findByPk to findById
...
router.post("/teacher/change-password", requireTeacher, async (req, res) => {
  try {
    ...
    if (newPassword.length < 6)
      throw new Error("Password must be at least 6 characters");
    // 4. Changed findByPk to findById
    const teacher = await Teacher.findById(req.session.teacher.id);
    if (!(await teacher.validatePassword(currentPassword)))
      throw new Error("Current password is incorrect");
    await teacher.update({
      password: await bcrypt.hash(newPassword, 10),
      mustChangePassword: false,
...
router.post("/teacher/forgot-password", async (req, res) => {
  ...
  if (!limit.allowed) {
    req.flash(
      "error",
      `Too many requests. Please wait ${limit.minutes} minute(s).`,
    );
...
router.get("/teacher/reset-password/:token", async (req, res) => {
  try {
    ...
    if (
      !verifyResetToken(req.params.token, teacher.email, teacher.resetToken)
    ) {
      req.flash("error", "Invalid reset link.");
      return res.redirect("/auth/teacher/forgot-password");
    }
...
router.post("/teacher/reset-password/:token", async (req, res) => {
  ...
  try {
    ...
    req.flash("error", err.message);
    res.redirect(
      `/auth/teacher/reset-password/${req.params.token}?email=${encodeURIComponent(req.body.email || "")}`,
    );
  }
});
