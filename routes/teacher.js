...
async function getTeacherClassStreamMap(teacherId) {
  ...
  rows.forEach((row) => {
    ...
    if (row.stream) {
      const existing = classStreamMap[cid].find(
        (s) => String(s.id) === String(row.stream.id),
      );
      if (!existing)
        classStreamMap[cid].push({
          id: String(row.stream.id),
...
router.get("/dashboard", async (req, res) => {
  try {
    ...
    const assignedClasses = teacherClassRows
      .map((tc) => {
        ...
        if (tc.streamId && clsJson.students) {
          clsJson.students = clsJson.students.filter(
            (s) => String(s.streamId) === String(tc.streamId),
          );
        }
    ...
    res.render("teacher/dashboard", {
      ...
      teacher: {
        ...
        teacherSubjects: teacherSubjectRows,
        classTeacherClasses,
      },
      today,
      canTakeAttendance,
...
router.get("/attendance", async (req, res) => {
  ...
  teacherClassRows.forEach((tc) => {
    ...
    if (tc.stream) {
      const existing = classStreamMap[cid].find(
        (s) => String(s.id) === String(tc.stream.id),
      );
      if (!existing)
        classStreamMap[cid].push({
          id: String(tc.stream.id),
          name: tc.stream.name,
        });
    }
  });
  ...
  if (dayOfWeek === 0 || dayOfWeek === 6) {
    canTakeAttendance = false;
    blockReason = "Attendance is not taken on weekends";
  }
  const holiday = await PublicHoliday.findOne({
    where: {
      date: today,
  ...
  if (req.query.classId) {
    const resolvedStreamId =
      selectedStreamId ||
      (await getTeacherAssignedStreamId(
...
router.post("/attendance/submit", async (req, res) => {
  try {
    ...
    if ([0, 6].includes(moment().day()))
      throw new Error("Cannot take attendance on weekends");
    const holiday = await PublicHoliday.findOne({
      where: {
        date: today,
      },
  ...
  res.redirect(
    `/teacher/attendance?classId=${req.body.classId}&streamId=${req.body.streamId || ""}`,
  );
});
...
router.get("/attendance/print", async (req, res) => {
  try {
    ...
    res.render("teacher/print-attendance", {
...
router.get("/attendance/history", async (req, res) => {
  ...
  if (req.query.classId && req.query.date) {
    const streamId = await getTeacherAssignedStreamId(
      req.session.teacher.id,
      req.query.classId,
    );
  ...
  teacherSubjectRows.forEach((ts) => {
    if (ts.class && ts.class.department) {
      ...
      if (!deptMap.has(dept.id))
        deptMap.set(dept.id, dept.toJSON ? dept.toJSON() : dept);
    }
  });
...
router.post("/marks/save", async (req, res) => {
  try {
    ...
    if (section === "fe") {
      ...
      const feIds = Object.keys(req.body)
      ...
      for (const studentId of studentIds) {
        ...
        const totalScore = calculateArtScore({
          homework,
          groupWork,
          quiz,
          classWork,
  ...
  res.redirect(
    `/teacher/marks?subjectId=${req.body.subjectId}&classId=${req.body.classId}&streamId=${req.body.streamId || ""}&term=${req.body.term}&year=${req.body.academicYear}`,
  );
});
...
async function loadStudentReportData(studentId, term, year, teacherId = null) {
  ...
  if (!student)
    throw new Error("Student record not found or not available for reporting");
  ...
  const reportComment =
    reportComments.find((c) => c.classId === student.classId) ||
    reportComments.sort(
...
router.get("/report-card/:studentId", (req, res) => {
  res.redirect(
    `/teacher/progressive-report/${req.params.studentId}?${new URLSearchParams(req.query).toString()}`,
  );
});
...
router.get("/comments", async (req, res) => {
  ...
  ctClasses.forEach((cls) => {
    ...
    if (!classStreamMap[cid]) classStreamMap[cid] = [];
    classStreamMap[cid].push({
      id: cls.streamId ? String(cls.streamId) : "",
      name: cls.stream ? cls.stream.name : "All streams",
    });
  });
  ...
  if (req.query.classId) {
    ...
    const teacherStreamId = await getTeacherAssignedStreamId(
      req.session.teacher.id,
      req.query.classId,
    );
    ...
    comments.forEach((c) => {
      existingComments[c.studentId] = c;
    });
...
router.post("/comments/save", async (req, res) => {
  ...
  res.redirect(
    `/teacher/comments?classId=${req.body.classId}&streamId=${req.body.streamId || ""}&term=${req.body.term}&year=${req.body.academicYear}`,
  );
});
...
router.post("/comments/generate", async (req, res) => {
  try {
    ...
    const ctRow = await TeacherClass.findOne({
      where: {
        teacherId: parseInt(req.session.teacher.id),
        classId: classIdInt,
        streamId: streamIdInt || null,
        isClassTeacher: true,
      },
    });
    if (!ctRow) {
...
Write only the report card comment text, nothing else.`;
    ...
    if (!apiKey || apiKey === "your_anthropic_api_key_here") {
      return res.status(500).json({
        error: "ANTHROPIC_API_KEY is not configured in your .env file",
      });
    }
    ...
    if (!comment) {
      ...
      return res.status(500).json({
        error: "No comment returned by AI. Response: " + JSON.stringify(data),
      });
    }
...
router.post('/timetable/:id/delete', async (req, res) => {
  try {
    // FIXED: Changed findByPk to findById
